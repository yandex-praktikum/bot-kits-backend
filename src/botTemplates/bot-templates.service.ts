...
export class BotTemplatesService {
    ...
    async create(createBotTemplateDto: CreateBotTemplateDto) {
        const botTemplate = await this.botTemplateModel.create(createBotTemplateDto);
    ...
    async update(id: string, updateBotTemplateDto: UpdateBotTemplateDto) {
        try {
            const result = await this.botTemplateModel.findByIdAndUpdate(id, updateBotTemplateDto, { new: true }).exec();
