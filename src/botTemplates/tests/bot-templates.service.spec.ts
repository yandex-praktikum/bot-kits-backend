...
describe('BotTemplates Service', () => {
    ...
    describe('update',  () => {
        it('should return a doc with new title', async () => {
            ...
            expect(await botTemplatesService.update(10, newProps)).toStrictEqual({...result, ...newProps, _id: 10});
            expect(repo.findByIdAndUpdate).toHaveBeenCalledTimes(1);
            expect(repo.findByIdAndUpdate).toHaveBeenCalledWith(10, newProps, {new: true});
        });
    });
