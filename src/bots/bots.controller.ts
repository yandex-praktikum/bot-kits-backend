...
import {botTemplates} from "./dto/constants/botTemplates";
...
export class BotsController {
	...
	async findMy(@Req() req): Promise<{ bots: Bot[], commands: TypeCommands[] }> {
		return Promise.resolve({
			bots: await this.botsService.findAllByUser(req.user.id).then(result => result.concat(botTemplates as Bot[])),
