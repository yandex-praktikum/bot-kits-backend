...
export class BotAccessesController {
  ...
  @ApiOperation({
    ...
    description: "Создает запись о доступе пользователя к боту, используется при создании бота"
  })
  @ApiCreatedResponse({
  ...
  @ApiOperation({
    ...
    description: "Позвоялет поделиться доступом по botId и создать новый доступ, если пользователь имеет уровень доступа super_admin к данному боту"
  })
  @ApiCreatedResponse({
